# tonkeeper-Larp
https://t.me/larpforfree https://t.me/eighty 

i just fucked my fusi up counting too much fusi's up we got beams on this fusi u dont gotta pray for us 🥺🥺🥺


# RUN SCRIPT TWICE THE FIRST TIME YOU USE IT!!!!!

# How tew use 
1. open toneeper
2. click control shift I
3. click console
4. paste scrapt
5. click enter
6. close devtools
7. BOOM UR RICH ASF..


also to get da walletz click add account then add a watch only addy then for the address go on tonviewer and find sum rich nigga Lol 
do this b4 running scrapt cruh
